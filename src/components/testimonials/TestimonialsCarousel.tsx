  ...
  return (
    <section className="py-16 bg-white">
      <h2 className="text-3xl font-bold text-center mb-8">
        O que dizem sobre o rebU98
      </h2>
      <div className="relative max-w-5xl mx-auto">
        <motion.ul
          ref={ref}
          style={{ maskImage, WebkitMaskImage: maskImage }}
          className="flex gap-4 overflow-x-auto px-2 scroll-smooth snap-x snap-mandatory"
          role="list"
          aria-label="Depoimentos dos usuários"
        >
          {testimonials.map((t, i) => (
           <motion.li
              key={i}
              role="listitem"
              className="bg-white shadow-md rounded-xl p-6 w-[250px] flex-shrink-0 text-center snap-start"
            >
              <img
                src={t.avatarUrl}
                alt={`Foto de ${t.nome}`}
